...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, onLogout }) => {
  const menuItems: { name: View; label: string; icon: string }[] = [
    ...
    { name: 'Staff', label: 'Personal', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
    { name: 'AI', label: 'Asistente AI', icon: 'M13 10V3L4 14h7v7l9-11h-7z' },
  ];
      ...
      <nav className="flex-1 mt-6 px-4 space-y-2">
        {menuItems.map((item) => (
          <button
            ...
            } ${item.name === 'AI' && currentView !== 'AI' ? 'border border-indigo-100 bg-indigo-50/30' : ''}`}
          >
            <svg className={`w-6 h-6 transition-colors duration-200 ${currentView === item.name ? 'text-white' : 'text-gray-400 group-hover:text-indigo-600'} ${item.name === 'AI' && currentView !== 'AI' ? 'text-indigo-500' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
