    ...
    <div className="space-y-6">
      ...
      {viewType === 'timeline' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {sortedReservations.map(res => {
            ...
            return (
              <button key={res.id} onClick={() => handleOpenView(res)} className="bg-white p-8 rounded-[3.5rem] border border-gray-100 shadow-sm text-left">
      ...
      {(modalMode === 'add' || modalMode === 'edit') && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xl flex items-center justify-center z-50 p-6" onClick={() => setModalMode(null)}>
          <div className="bg-white rounded-[4rem] p-10 max-w-xl w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
            <h2 className="text-3xl font-black text-gray-900 mb-8 text-center">{modalMode === 'add' ? 'Nueva Estancia' : 'Editar Estancia'}</h2>
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-1"><label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Huésped</label><select value={formData.guest_id} onChange={e => setFormData({...formData, guest_id: e.target.value})} className="w-full p-4 bg-slate-50 border rounded-2xl"><option value="">Seleccionar</option>{guests.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}</select></div>
              <div className="space-y-1"><label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Habitación</label><select value={formData.room_id} onChange={e => setFormData({...formData, room_id: e.target.value})} className="w-full p-4 bg-slate-50 border rounded-2xl"><option value="">Seleccionar</option>{rooms.map(r => <option key={r.id} value={r.id}>Hab. {r.number}</option>)}</select></div>
              <div className="space-y-1"><label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Entrada</label><input type="date" value={formData.check_in} onChange={e => setFormData({...formData, check_in: e.target.value})} className="w-full p-4 bg-slate-50 border rounded-2xl" /></div>
              <div className="space-y-1"><label className="text-[10px] font-black uppercase tracking-widest text-gray-400">Salida</label><input type="date" value={formData.check_out} onChange={e => setFormData({...formData, check_out: e.target.value})} className="w-full p-4 bg-slate-50 border rounded-2xl" /></div>
            </div>
            <div className="flex space-x-6 pt-10">
              <button onClick={() => setModalMode(null)} className="flex-1 py-5 bg-slate-50 rounded-2xl font-black text-slate-400">Cerrar</button>
              <button onClick={handleSave} className="flex-1 py-5 bg-indigo-600 text-white rounded-2xl font-black shadow-lg">Sincronizar</button>
            </div>
          </div>
        </div>
      )}
